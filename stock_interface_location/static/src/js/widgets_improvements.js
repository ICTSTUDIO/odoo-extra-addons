function openerp_picking_widgets(instance){
    ...
    module.PickingEditorWidget = instance.web.Widget.extend({
        ...
        get_header_source: function(){
            return this.getParent().get_header_source();
        },
        get_header_destination: function(){
            return this.getParent().get_header_destination();
        },
        ...
        get_rows: function(){
            ...
            _.each( model.packoplines, function(packopline){
                ...
                if (packopline.result_package_id[1] !== undefined && $.inArray(packopline.result_package_id[0], pack_created) === -1){
                    ...
                    self.rows.push({
                        cols: { product: packopline.result_package_id[1],
                            ...
                            availability: '',
                            ordered_qty: '',
                            picker: packopline.write_uid[1],
                            pick_datetime: packopline.write_date,
                        },
                ...
                self.rows.push({
                    cols: { product: packopline.product_id[1] || packopline.package_id[1],
                        ...
                        ul_id: -1,
                        product_location: packopline.product_location ? packopline.product_location : "",
                        remark: packopline.remark ? packopline.remark : "",
                        ...
                        ordered_qty: packopline.stock_move ? packopline.stock_move.product_qty : "",
                        picker: packopline.write_uid[1],
                        pick_datetime: packopline.write_date,
                    },
            ...
            group_by_container = _.groupBy(self.rows, function(row){
                return row.cols.product_location;
            });
            var sorted_row = [];
        ...
        renderElement: function(){
            ...
            this.$('.js_remark').blur(function(){
                var op_id = $(this).parents("[data-id]:first").data('id');
                var value = $(".js_remark").val();
                if (value!==""){
                    self.getParent().set_operation_remark(value, op_id);
                }
            });
    ...
    module.PickingMainWidget = module.MobileWidget.extend({
        ...
        get_header_source: function(){
            if(this.picking){
                return this.picking.location_id[1];
            }else{
                return '';
            }
        },
        get_header_destination: function(){
            if(this.picking){
                return this.picking.location_dest_id[1];
            }else{
                return '';
            }
        },
        ...
        set_operation_remark: function(remark, op_id){
            var self = this;
            if(remark != ""){
                return new instance.web.Model('stock.pack.operation')
                    .call('write',[[op_id],{'remark': remark }])
                    .then(function(){
                        self.refresh_ui(self.picking.id);
                    });
            }
