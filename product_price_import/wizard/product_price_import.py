...
class ProductPriceImport(models.TransientModel):
    _name = 'product.price.import'
    _description = 'Product Price Import'

    product_price_data = fields.Binary(string='File', required=True)
    product_price_fname = fields.Char(string='Filename')
    lines = fields.Binary(
    ...
    @api.one
    @api.depends('product_price_data')
    def _compute_lines(self):
        if self.product_price_data:
            lines = base64.decodestring(self.product_price_data)
    ...
    @api.onchange('product_price_data')
    ...
    def _onchange_csv_separator(self):
        if self.csv_separator and self.product_price_data:
    ...
    def product_price_import(self):
        ...
        for line in reader:
            ...
            if line.get('productcode') and line.get('verkoopprijs'):
                product = self.env['product.product'].search([('default_code', '=', line.get('productcode'))])
                if product:
                    product.write({'list_price': str2float(line.get('verkoopprijs'),self.decimal_separator)})
