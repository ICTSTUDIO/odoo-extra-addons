...
class ProductCostpriceImport(models.TransientModel):
    _name = 'product.costprice.import'
    _description = 'Product Costprice Import'

    product_costprice_data = fields.Binary(string='File', required=True)
    product_costprice_fname = fields.Char(string='Filename')
    lines = fields.Binary(
    ...
    @api.one
    @api.depends('product_costprice_data')
    def _compute_lines(self):
        if self.product_costprice_data:
            lines = base64.decodestring(self.product_costprice_data)
    ...
    @api.onchange('product_costprice_data')
    ...
    def _onchange_csv_separator(self):
        if self.csv_separator and self.product_costprice_data:
    ...
    @api.multi
    def product_costprice_import(self):
